...
2. In VSCode, modify `app.py` with the extra line in `app_ui` and the new function `my_sumstats()`
...
## Reactive Functions: Normal Distribution Example
- Can we resolve this just by deleting one of the `sample` definitions? 
- Let's try removing it from `my_sumstats()`:
...
- **Reactive functions** are functions that reactively run when one of their dependencies change
...
1. Create skeleton `covid/app.py`
...
## COVID Data Example: Starting point 
- In student repo `shiny_11` you have `covid/nyt_covid19_data.csv`
- Create an empty text file in that same folder called `app.py`
...
df = pd.read_csv("covid/nyt_covid19_data.csv") 
print(df.head())
...
- Hard-coding every state name is not ideal. Next lecture, we'll discuss how to make it more 'reactive'
